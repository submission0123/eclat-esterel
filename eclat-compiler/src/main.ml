...
let () =
  ...
  Arg.parse [
    ...
    ("-Wsize",   Arg.Set BHDL_typing.emit_warning_flag,
    ...
    ("-causality", Arg.Set Compile.check_causality_flag, "run causality analysis");
    ]
      add_input "Usage:\n  ./eclat file"
;;
...
let main () : unit =
  ...
  let args = List.map (Gen_BHDL.to_a ~genv:pi.genv) arg_list in
