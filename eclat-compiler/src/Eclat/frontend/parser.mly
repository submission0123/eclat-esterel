...
%token NODE IMPLY MINUS_LCUR RCUR_MINUS_GT MINUS_BRACK RBRACK_MINUS_GT
...
| ty=ty_tuple_next MINUS_BRACK d=dur RBRACK_MINUS_GT ty2=ty_tuple_next {
...
| sz=size MINUS n=INT_LIT { Sz_add(sz,-n) }
| n=INT_LIT TIMES sz=size
| sz=size TIMES n=INT_LIT { 
     Sz_mul(n,sz)
 }
| n=INT_LIT HAT sz=asize { 
  ...
  RBRACKET
        { let es' = e1::es in
          E_letIn(group_ps (List.map (fun _ -> P_unit) es'), 
                  Types.new_ty_unknown(), E_par(gensym(),es'),E_const Unit)
        }
...
| w=bindings_and(P,E)
  { match w with
    | [],_ | _,[] -> assert false
    | [p],[e] -> (p,e)
    | ps,es -> (P_tuple ps, E_par(gensym(),es)) }
...
| x=IDENT { let open Ast_mk in
            ...
            match x with
            | "ref" -> mk @@ E_const Ref
            | "pause" -> E_pause (Ast.gensym(), mk@@E_const(Unit))
