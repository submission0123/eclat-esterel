...
let rec map_under_exec_and_par e =
  ...
  let rec aux env e =
    ...
    | E_par(l,es) ->
        E_par(l,List.map (fun e -> map_under_exec_and_par (decl env e)) es)
    | E_exec(e1,e0,eo,l) ->
    ...
    | E_letIn(P_var x,tyB,E_trap(tyB',l), e1) ->
        E_letIn(P_var x,tyB,E_trap(tyB',l), map_under_exec_and_par (decl env e1)) *)
    | E_suspend(q,e1,x) ->
        E_suspend(q,map_under_exec_and_par (decl env e1),x)
    | e -> 
