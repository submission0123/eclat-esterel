...
let rec lfloat (e:e) : e =
  ...
  | E_letIn(P_var x,ty,E_trap(tyB,l),e2) ->
      [],E_letIn(P_var x,ty,E_trap(tyB,l),lfloat e2)
  | E_letIn(p,ty,e1,e2) ->
      let ds1,e1' = glob e1 in
      let e' = lfloat e2 in
      ds1@[(p,ty,e1')],e'
  ...
  | E_par(l,es) ->
      (*let dss,es' = List.split (List.map glob es) in
      (* let rec loop acc dss =
        match ds with
        | [] -> acc
        | ds::dss' -> List.partition (fun (_,E_sig_create _)  *)
      ds1,E_par(l,es')*)
       [],E_par(l,List.map lfloat es) 
