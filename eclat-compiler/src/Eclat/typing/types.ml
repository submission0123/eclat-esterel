...
let show_wcet_exp_flag = ref true
...
| Sz_add(sz,n) ->
   (match canon_size sz with
    ...
    | Sz_add(sz',k) -> canon_size @@ Sz_add(sz',n+k)
    | sz -> Sz_add (sz,n))
| Sz_mul(0,sz) -> Sz_lit 0
| Sz_mul(1,sz) -> canon_size sz
| Sz_mul(n,sz) ->
    (match canon_size sz with
    | Sz_lit m as sz' -> 
        let k = n*m in
    ...
    | Sz_mul(m,sz') -> 
        let k = n*m in
        if k >= m (* avoid overflow *)
        then canon_size @@ Sz_mul(k,sz')
        else sz'
    | Sz_pow2(sz'') as sz0 -> if n mod 2 = 0 then Sz_mul(n/2,Sz_pow2(Sz_add(sz'',1)))
                              else Sz_mul(n,Sz_pow2(sz0))
    | sz' -> Sz_mul (n,sz'))
| Sz_pow2(sz) ->
    (match canon_size sz with
    | Sz_lit m ->
        if m < 0 || m >= 63 (* avoid overflow *)
        then Sz_pow2(sz)
        else if m = 0 then Sz_lit(1) 
        else Sz_lit(1 lsl m)
    | sz -> Sz_pow2 (sz))
...
let pp_size fmt (sz:size) : unit =
  ...
  | Sz_mul(n,sz) -> fprintf fmt "(%d * %a)" n pp sz
...
and instance_aux ~new_unknown
  ...
  | Sz_mul(n,sz) -> Sz_mul(n,inst_size sz)
...
| Sz_mul(n,sz) -> Sz_mul(n,rename_size unknowns sz)
...
| Sz_mul(n,sz) -> Sz_mul(n,subst_size vsize sz)
