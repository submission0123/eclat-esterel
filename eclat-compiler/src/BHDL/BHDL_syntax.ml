...
module Debug = struct
  ...
  | TSize_mul(n,size) -> fprintf fmt "(%d * %a)" n pp_ty size
...
let pp_vector = Ast_pprint.pp_vector
  ...
  | S_array_from_file(y,a) ->
      fprintf fmt "array_from_file(%s,%a)" y pp_a a
  | S_seq(s1,s2) ->
