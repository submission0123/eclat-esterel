...
let rec pp_call operators typing_env res fmt (op,a) =
  ...
  match op with
  | GetTuple(i,_,ty) ->
      let ts = match BHDL_typing.canon ty with TTuple ts -> ts | _ -> assert false in
      (match a with
...
| A_record_field(x,y,t) ->
    let b_list = match BHDL_typing.canon t with
...
| S_skip -> fprintf fmt "()"
| S_continue(id,q) -> fprintf fmt "state_%a := %a" pp_ident id pp_state q
| S_if(z,s1,so) ->
