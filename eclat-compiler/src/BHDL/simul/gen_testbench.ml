...
let gen_testbench fmt ?(vhdl_comment="") ~genv typing_env name ty ((argument,ta),(result,tr)) (args_for_simul: _ list) =
  ...
  let ss_args = List.map (fun arg_for_simul ->
      let s_arg = Flat_let_atom.flat_s (BHDL_syntax.set_ tmp arg_for_simul) in
      BHDL_typing.typing_error_handler @@ (fun () ->
        Option.iter (fun t -> BHDL_typing.add_typing_env typing_env_tb tmp t) ta;
        BHDL_typing.typing_s ~genv ~result:tmp typing_env_tb s_arg;
