...
let ptr_taken x = BHDL_typing.ptr_taken x
  ...
  | Runtime(External_fun (x,ty)) ->
      ...
      (* let rec extract_tyB tyB =
        ...
        | TyB_abstract(_,_sz,tyB_list) ->
            List.map (fun tyB -> BHDL_typing.(size_ty (translate_tyB tyB))) tyB_list
      ...
      if annot_with_sizes then (
               let extra = match Types.canon_ty ty with
                  | Ty_fun(Ty_base tyB1,_,tyB2) -> 
                       [size_ty BHDL_typing.(translate_tyB tyB1)  
                       ;size_ty BHDL_typing.(translate_tyB tyB2)  ]
                  | _ -> assert false
               in
...
let print_external fmt (n,(ty,shared,_)) =
  ...
  | Types.Ty_fun(arg,d,ret) -> 
      size_ty BHDL_typing.(translate_ty arg),d, size_ty BHDL_typing.(translate_tyB ret)
  | _ -> assert false
  in
