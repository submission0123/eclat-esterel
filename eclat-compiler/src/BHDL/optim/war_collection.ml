...
let collect_a a =
  ...
  | A_array_get(x,y) ->
     SMap.add x () (SMap.add y () m)
  | A_ptr_taken(x) -> SMap.add (BHDL_typing.ptr_taken x) () m
...
let rec compute s = (* check s @@ *)
  ...
  | S_acquire_lock(x)
  | S_release_lock(x) ->
      let m = SMap.singleton (BHDL_typing.ptr_taken x) () in
      (m,m,m)
  | S_read_start(_,a) ->
      let r = collect_a a in
      (r, empty, empty)
...
let collect_main ~rdy ~result (ts,s) =
  let unused_id = "%" in
  let w,r,war = compute @@ S_fsm(unused_id,rdy,result,unused_id,ts,s) in
  war (* not complete, return [union w war] to avoid latches in synthesis *)
